...
router.route("/process/login").post(function (req, res) {
  ...
  if (req.session.user) {
    ...
    res.write(
      "<br><br><button><a href='/photomulti3205.html'>파일업로드로 이동하기</a></button>"
    );
...
// 로그인 후 세션 저장
router
  .route("/process/photo12")
  .post(upload.array("photo12", 12), function (req, res) {
    console.log("/process/photomulti3205 호출됨");
    ...
    try {
      ...
      if (Array.isArray(files)) {
        //배열에 들어간 경우 - 설정에서 1개의 파일도 배열에 넣게 함
